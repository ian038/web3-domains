...
contract Domains is ERC721URIStorage {
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 2 * 10**17; // 2 MATIC = 2 000 000 000 000 000 000 (18 decimals).
        } else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        ...
        console.log(
            "Registering %s.%s on contract with tokenID %d",
            name,
            tld,
            newRecordId
        );
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Spartan name service", "image": "data:image/svg+xml;base64,',
